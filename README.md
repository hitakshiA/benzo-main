<p align="center">
  <img src="backend/assets/readme/benzo-readme-banner.png" alt="Benzo" />
</p>
...
This is the consolidated Benzo monorepo. Every top-level folder is a standalone
project that keeps its own README; clone or open any one of them independently.

```text
backend/    eERC contracts + 5 Groth16 verifiers, the Fastify/Postgres API, and BenzoNet L1 infra  (the core)
console/    Business console, desktop-first managed workspace: treasury, payroll, approvals, audit
wallet/     Consumer wallet, mobile-first self-custody: shield, send, prove, cash out
landing/    Marketing landing site for Benzo
explorer/   Branded BenzoNet L1 block explorer (Blockscout)
```

| Folder | What it is | Upstream |
| --- | --- | --- |
| [`backend/`](backend) | eERC contracts, the API service, and BenzoNet L1 infra | [Miny-Labs/benzo](https://github.com/Miny-Labs/benzo) |
| [`console/`](console) | Business console (managed, on BenzoNet L1) | [Miny-Labs/benzo-console](https://github.com/Miny-Labs/benzo-console) |
| [`wallet/`](wallet) | Consumer wallet (self-custody, Fuji + C-Chain mainnet) | [Miny-Labs/benzo-wallet](https://github.com/Miny-Labs/benzo-wallet) |
| [`landing/`](landing) | Landing page | [Miny-Labs/benzo-landing](https://github.com/Miny-Labs/benzo-landing) |
| [`explorer/`](explorer) | BenzoNet L1 block explorer | [Miny-Labs/benzo-explorer](https://github.com/Miny-Labs/benzo-explorer) |

Each app folder has its own setup steps. The rest of this document is the backend
and protocol overview, and its paths are relative to [`backend/`](backend).
...
Source of truth: [`contracts/deployments/avalanche.json`](backend/contracts/deployments/avalanche.json)
and [`packages/config/src/deployments/avalanche.json`](backend/packages/config/src/deployments/avalanche.json).
A per-network breakdown for all three chains lives in [`docs/DEPLOYMENTS.md`](backend/docs/DEPLOYMENTS.md).
...
[`contracts/deployments/fuji.json`](backend/contracts/deployments/fuji.json). This
...
· full manifest: [`contracts/deployments/benzonet.json`](backend/contracts/deployments/benzonet.json).
...
record is in [`docs/MAINNET_GO_NO_GO.md`](backend/docs/MAINNET_GO_NO_GO.md).
...
The `backend/` folder holds the Benzo contracts, service layer, and L1 infra.

```text
backend/contracts/       Hardhat workspace: eERC, verifiers, Benzo registries, deployment manifests (fuji / avalanche / benzonet)
backend/services/api/    Fastify + Postgres service for auth, onboarding, activity, org workflows
backend/infra/           BenzoNet genesis, deployed L1 metadata, edge/Caddy topology, smoke tests
backend/packages/config/ Shared chain defs, deployed addresses, and circuit URL helpers
backend/assets/readme/   README banner and brand marks
```

The end-user apps (`wallet/`, `console/`, `landing/`) and the L1 explorer
(`explorer/`) each live in their own top-level folder in this repo; see
...
[`contracts/README.md`](backend/contracts/README.md) and
[`infra/README.md`](backend/infra/README.md).
...
[`ava-labs/EncryptedERC`](backend/contracts/contracts/eerc/VENDOR.md) code keeps its
