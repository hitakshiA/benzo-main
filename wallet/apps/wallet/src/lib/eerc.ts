...
// new wallet and only then proceeds regardless.
//
// Sized against Fuji's post-Helicon base fee (~10 wei): a registration costs about
// 5e-11 AVAX and a shield about 1.2e-7, so 0.0001 still covers a full
// register/shield/send/unshield cycle several hundred times over. Testnet AVAX is
// the scarce resource here, not gas — the old 0.01 gate forced a 0.03 drip that was
// ~150 million times what the transactions actually consume.
const FAUCET_MIN_GAS_WEI = 100_000_000_000_000n; // 0.0001 AVAX
